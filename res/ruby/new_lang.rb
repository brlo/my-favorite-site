...
::ImportVerse.all.order('verses.id').find_each do |s|
  ...
  str = str.gsub(/^\s|\s$/, '')
  # если там в итоге ничего не осталось, то поставить прочерк (иначе будет ошибка сохранения)
  str = str.presence || '-'
...
# ###############################################################################
 ...
 # Для импорта используй запрос { "lang" : { "$in" : [ "en-nrsv" ] } }
