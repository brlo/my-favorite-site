...
function clickForUpdate() {
  emit('forUpdate', props.item.obj)
}
</script>

<template>
<div>
  <a @click.prevent="clickForUpdate">{{ item.obj.title }}</a> ({{ item.obj.priority }})
  <a @click.prevent="destroy" class="menu-destroy-icon">
  ...
  <div class="menu-childs">
    <MenuItem
      ...
      :item="child"
      @destroy="(items) => { emit('destroy', items) }"
