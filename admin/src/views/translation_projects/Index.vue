...
function autoSearch() {
  ...
  api.get('/translation_projects/', params).then(data => {
    isLoading.value = false;
    console.log(data)
    if (data.success == 'ok') {
      translationProjects.value = data.items;
    } else {
...
<template>
<h2 v-if="isListOnly">Недавние проекты преводов</h2>
<h2 v-else="isListOnly">Статьи</h2>

<router-link v-if="currentUser?.privs?.pages_create && !isListOnly" :to="{ name: 'NewTranslationProject' }">
...
<div v-if="translationProjects.length == 0">
  <div class='page'>Ничего не найдено</div>
</div>

<div id="pages" v-for="project in translationProjects">
  <div class='page'>
    {{ langs[project.lang] }}
    <router-link :to="{ name: 'ShowTranslationProject', params: { id: project.id }}">
      {{ project.title }}
    </router-link>
  </div>
</div>
