module Api
  class PagesController < ApiApplicationController
    ...
    def list
      ...
      # - УБРАТЬ ОТ СЮДА УДАЛЁННЫЕ СТАТЬИ, ЧТОБЫ ОНИ НЕ ПОКАЗЫВАЛИСЬ В ПОДСКАЗКАХ, НО ОСТАЛИСЬ НА ГЛАВНОЙ СВОДЕ И В ПОИСКЕ С МЕТКАМИ
      # - В СВОДКЕ ПОКАЗЫВАТЬ ТОЛЬКО ТЕ ПР, КОТОРЫЕ ЕЩЁ НЕ ПРИНЯТЫ
      # - ПОКАЗАТЬ СЛОВАРЬ ДВОРЕЦКОГО
      # - СОСТАВИТЬ ОГЛАВЛЕНИЕ ПО H2, в бэкенде
      # - сделать предложение создать страницу (с учётом языка и род. страницы)
      @pages = ::Page.
        includes(:user).
        only(
