module Api
  class PagesController < ApiApplicationController
        ...
        if @page.save
          ...
          if menu_id
            ...
            if @page.parent&.is_page_menu?
              # СОЗДАНИЕ РОДИТЕЛЬСКОГО ЭЛЕМЕНТА МЕНЮ, если указан params[:page][:menu_category]
              ...
              # СОЗДАНИЕ ЭЛЕМЕНТА МЕНЮ, ВЕДУЩЕГО К СОЗДАВАЕМОЙ СТРАНИЦЕ
              # это действие происходит всегда при создании страницы.
              # Даже если ничего не указали, в родительской странице должен появиться пункт меню, а там уже хозяин его распределит.
              # Если же указали params[:page][:menu_item_name], то этой менюшке пропишется имя из этого параметра.
              ...
              # Если название статьи заканчивается на #1, то помимо менюшки со ссылкой на эту страницу,
              # надо также создать подменюшку с названием 1.
              # В итоге получится 2 элемента меню: Один — хозяин с названием статьи (без #1), и у него один потомок с названием "1",
              # оба ведут на одну и ту же статью. А дальше скрипт уже будет создавать следующие части: #2, #3 и т.д.
              # Там уже не потребуется такая дополнительная обработка.
              if @page.title =~ /#1$/
                new_sub_item = ::Menu.create!(
                  page_id: @page.parent_id, # страница-хозяин элемента меню
                  parent_id: new_item.id, # родительский элемент
                  title: "1",
                  path: @page.path,
                  priority: 1,
                )
    ...
    def page_owner?
      if @page.present?
        ::Current.user.pages_owner.to_a.include?(@page.id) ||
        ::Current.user.pages_owner.to_a.include?(@page.parent_id)
      end
    end
